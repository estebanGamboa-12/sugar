...
class Timeline {
  ...
  to(target: unknown, vars?: TweenVars, _position?: string | number) {
    applyVars(target, vars);
    return this;
  }

  fromTo(target: unknown, _from: TweenVars, to: TweenVars, _position?: string | number) {
    return this.to(target, to);
  }

  call(callback: () => void, _params?: unknown[], _position?: string | number) {
    callback();
    return this;
  }
}
...
function applyVars(target: unknown, vars?: TweenVars) {
  ...
  elements.forEach((item, index) => {
    ...
    if (vars.autoAlpha !== undefined || vars.opacity !== undefined) {
      const raw = vars.opacity ?? vars.autoAlpha;
      const value = Number(resolve(raw, index, item));
    ...
    if (vars.y !== undefined) {
      const value = Number(resolve(vars.y, index, item)) || 0;
      style.transform = `translateY(${value}px)`;
    }
    if (vars.color !== undefined) {
      style.color = String(resolve(vars.color, index, item));
    }
    if (vars.backgroundColor !== undefined) {
      style.backgroundColor = String(resolve(vars.backgroundColor, index, item));
    }
  });
}

const gsap = {
  registerPlugin: (...args: unknown[]) => args,
  set: (target: unknown, vars?: TweenVars) => applyVars(target, vars),
  to: (target: unknown, vars?: TweenVars) => {
    applyVars(target, vars);
    return { kill: () => undefined, scrollTrigger: { kill: () => undefined } };
  },
  fromTo: (target: unknown, _from: TweenVars, to: TweenVars) => {
    applyVars(target, to);
    return { kill: () => undefined, scrollTrigger: { kill: () => undefined } };
  },
  ...
  matchMedia: () => ({
    add: (_query: string, fn: () => void | (() => void)) => fn(),
    revert: () => undefined,
  }),
  utils: {
    toArray: <T>(selector: string) => Array.from(document.querySelectorAll(selector)) as T[],
  },
};
